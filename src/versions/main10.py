        ...
        for _, event in events.iterrows():
            with st.expander(f"{event['Event Name']} on {event['Date']}"):
                ...
                if main_list.empty:
                    ...
                    st.write("**Main List**")
                    for _, reg in main_list.iterrows():
                        user = users[users['ID'] == reg['UserID']].iloc[0]
                        st.write(f"{user['Name']} {user['Last Name']} ({user['Username']})")

                if reserve_list.empty:
                    st.write("No users in reserve list for this event.")
                else:
                    st.write("**Reserve List**")
                    for _, reg in reserve_list.iterrows():
                        user = users[users['ID'] == reg['UserID']].iloc[0]
                        st.write(f"{user['Name']} {user['Last Name']} ({user['Username']})")
